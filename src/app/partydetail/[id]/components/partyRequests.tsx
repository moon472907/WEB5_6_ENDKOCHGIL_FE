...
import { fetchPartyRequests, PartyRequestItem } from '@/lib/api/parties/partyrequests';
...
export default function PartyRequests({ partyId }: Props) {
  ...
  return (
    <div className="min-w-[320px] space-y-3">
      ...
      {!loading && !error && items.length > 0 && (
        <ul className="overflow-hidden rounded-xl border border-gray-200 divide-y divide-gray-100">
          {items.map((u) => (
            <li key={u.id} className="flex items-center justify-between gap-3 bg-basic-white p-3">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-[11px] text-gray-500">{u.status}</div>
              </div>
              {/* 승인/거절 버튼 */}
              <PartyRequestDecision
                partyId={partyId}
                request={u}
                onApprovedAction={(memberId) => setItems(prev => prev.filter(x => x.id !== memberId))}
                onRejectedAction={(memberId) => setItems(prev => prev.filter(x => x.id !== memberId))}
              />
            </li>
          ))}
        </ul>
